...
export class DatabaseService {
  ...
  updateWeekObject(weekPrefs) {
    let weeks =
      typeof weekPrefs === "string" ? JSON.parse(weekPrefs) : { ...weekPrefs };

    const currentWeek = String(this.getYearWeek()); //FIX
    console.log("updateWeekObject", 1, currentWeek);

    if (weeks.hasOwnProperty(currentWeek)) {
      weeks[currentWeek] = Number(weeks[currentWeek]) + 1;
      console.log("This is the current week updated", weeks);
    } else {
      weeks[currentWeek] = 1;

      // Remove oldest week safely
      const sortedKeys = Object.keys(weeks).sort();
      if (sortedKeys.length > 4) {
        delete weeks[sortedKeys[0]];
      }

      console.log("This is the Week when not current week", weeks);
    }

    console.log("This is Post On the Main updateWeekObject", weeks);
    return weeks;
  }
  ...
  updatePostPrefs({ prevousData = {}, currentData = {}, Prefs = {} }) {
    ...
    if (prevousStatus == "Draft" && currentStatus == "Post") {
      ...
      if (prevousVisibilty == "Public" && currentVisibilty == "Public") {
        ...
        prevousVisibilty == "Private" &&
        currentVisibilty == "Private"
      ) {
        newDataPrefs.Private += 1;
      } else if (
        prevousVisibilty == "Private" &&
        currentVisibilty == "Public"
      ) {
        newDataPrefs.Private -= 1;
        newDataPrefs.Public += 1;
      } else if (
        prevousVisibilty == "Public" &&
        currentVisibilty == "Private"
      ) {
        newDataPrefs.Private += 1;
        newDataPrefs.Public -= 1;
  ...
  async createPost({
    ...
    try {
      ...
      if (document) {
        let mainUpdateLable = await authService.updateLabels({
    ...
    try {
      ...
      if (defaults || types == "private") {
        ...
        if (privatePost.documents.length === limit) {
          privateCur =
            privatePost.documents[privatePost.documents.length - 1].$id;
        } else {
          privateCur = null;
        }
        console.log(
  ...
  async deleteFile({ FileId }) {
    ...
    try {
      const res = await this.bucket.deleteFile(env.APPWRITE_BUKET_ID, FileId);
